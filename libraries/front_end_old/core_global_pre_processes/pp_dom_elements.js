const GLOBAL_ID_PAUSED_BACKGROUND_FILTER     = 'a'; // #pre-process_global_constant
const GLOBAL_ID_PAUSE_DISPLAY                = 'b'; // #pre-process_global_constant
const GLOBAL_ID_SUB_PAUSED_DISPLAY_TITLE     = 'c'; // #pre-process_global_constant
const GLOBAL_ID_SUB_PAUSED_DISPLAY_SUB_TITLE = 'd'; // #pre-process_global_constant
const GLOBAL_ID_CURSOR_ICON                  = 'e'; // #pre-process_global_constant
const GLOBAL_ID_CANVAS_MAIN                  = 'h'; // #pre-process_global_constant

const GLOBAL_ID_HUD_DEBUG                    = 'l'; // #pre-process_global_constant
const GLOBAL_ID_HUD_TYPING                   = 'n'; // #pre-process_global_constant
const GLOBAL_ID_HUD_CHAT                     = 'm'; // #pre-process_global_constant
const GLOBAL_ID_DATE_TIME                    = 'o'; // #pre-process_global_constant


const TEXT_ALIGNMENT_START  = 0; // #pre-process_global_constant
const TEXT_ALIGNMENT_CENTER = 1; // #pre-process_global_constant
const TEXT_ALIGNMENT_END    = 2; // #pre-process_global_constant

// TEMPORARY LOCATION.
//https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/globalCompositeOperation
const CANVAS_RENDERING_STYLE_DEFAULT = 'source-over';
const CANVAS_RENDERING_STYLE_OVERLAP = 'destination-atop';

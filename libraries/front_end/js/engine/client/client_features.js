...
$_QE.prototype.ClientFeatures = function() {
    ...
    // Not required.
    if (navigator.getVRDisplays !== undefined && navigator.getVRDisplays().length !== 0) {
        this._features[CLIENT_FEATURE_VR] = 1;
    } else {
    ...
    this.are_required_features_enabled = function() {
        return this.is_feature_enabled(CLIENT_FEATURE_CANVAS) && this.is_feature_enabled(CLIENT_FEATURE_WEBGL);
    };

    this.get_names_of_required_features_not_enabled = function() {
        let names = '[';
        if (!this.is_feature_enabled(CLIENT_FEATURE_CANVAS)) {
            names += 'Canvas, ';
        }
        if (!this.is_feature_enabled(CLIENT_FEATURE_WEBGL)) {
            names += 'WebGL, ';
        }
        return names + ']';
    };
...
$_QE.prototype.ClientFeatureMobile = function() {
    let is_mobile = false;
    // From : https://stackoverflow.com/questions/3514784/what-is-the-best-way-to-detect-a-mobile-device-in-jquery
    ...
    $_QE.prototype.ClientFeature.call(this, CLIENT_FEATURE_MOBILE, 'Mobile', false, is_mobile);
};

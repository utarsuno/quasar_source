...
if(properties.hasOwnProperty(ENTITY_DEFAULT_PROPERTY_CHILD_IDS)){value=properties[ENTITY_DEFAULT_PROPERTY_CHILD_IDS];if(is_string(value)){this.ep_child_ids=eval(value);}else{this.ep_child_ids=value;}}else{this.ep_child_ids=[];this.children=[];}
if(properties.hasOwnProperty(ENTITY_DEFAULT_PROPERTY_PARENT_IDS)){value=properties[ENTITY_DEFAULT_PROPERTY_PARENT_IDS];if(is_string(value)){this.ep_parent_ids=eval(value);}else{this.ep_parent_ids=value;}}else{this.ep_parent_ids=[];this.parents=[];}
...
var non_default_properties=this.get_all_non_default_properties();for(var key in properties){if(properties.hasOwnProperty(key)){if(key!==ENTITY_DEFAULT_PROPERTY_RELATIVE_ID&&key!==ENTITY_DEFAULT_PROPERTY_CHILD_IDS&&key!==ENTITY_DEFAULT_PROPERTY_PARENT_IDS&&ENTITY_DEFAULT_PROPERTY_TYPE){this.set_property(key,properties[key]);}}}
...
this.needs_to_be_saved=true;},has_property:function(property_name){return this.hasOwnProperty(property_name);},get_relative_id:function(){return this[ENTITY_DEFAULT_PROPERTY_RELATIVE_ID];},get_type:function(){return this[ENTITY_DEFAULT_PROPERTY_TYPE];},get_value:function(property_name){return this[property_name];},get_name:function(){return this.name;},get_all_non_default_properties:function(){var properties={};var all_keys=Object.keys(this);for(var i=0;i<all_keys.length;i++){if(all_keys[i].startsWith(ENTITY_PROPERTY_START_TOKEN)){properties[all_keys[i]]=this.get_value(all_keys[i]);}}
return properties;},get_all_properties:function(){var properties={};var all_keys=Object.keys(this);for(var i=0;i<all_keys.length;i++){if(all_keys[i].startsWith(ENTITY_PROPERTY_START_TOKEN)){properties[all_keys[i]]=this.get_value(all_keys[i]);}}

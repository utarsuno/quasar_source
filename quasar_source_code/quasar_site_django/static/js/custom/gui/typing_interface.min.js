...
this.needs_another_update=needs_one_more_update;},add_chat_message:function(message){this.add_message(message,MESSAGE_TYPE_PLAYER_CHAT);},add_message:function(text_message,text_color){this.messages.unshift([text_message,text_color]);this.update();},add_user_text:function(){var current_input=this.input_text.get_text();if(current_input.length>0){if(current_input[0]==='.'){switch(current_input){case'.save':this.add_server_message('Saving changes to the server!');MANAGER_ENTITY.update_server_and_database();break;}}else{MANAGER_WORLD.player.send_chat_message(current_input);}
}},add_server_message:function(server_message){this.add_message('server : '+server_message,MESSAGE_TYPE_SERVER);}};
...
this.needs_another_update=needs_one_more_update;},add_chat_message:function(message){this.add_message(message,MESSAGE_TYPE_PLAYER_CHAT);},add_message:function(text_message,text_color){this.messages.unshift([text_message,text_color]);this.update();},add_user_text:function(){var current_input=this.input_text.get_text();if(current_input.length>0){this.add_message(current_input,MESSAGE_TYPE_USER);MANAGER_WORLD.player.send_chat_message(current_input);}},add_server_message:function(server_message){this.add_message(server_message,MESSAGE_TYPE_SERVER);}};
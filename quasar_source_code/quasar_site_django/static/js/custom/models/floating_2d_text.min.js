...
return this.text;},get_position:function(){return this.object3D.position;},state_change_look_at:function(being_looked_at){if(being_looked_at){this.wireframe.material.color.setHex(COLOR_HIGHLIGHT);this.update_color(COLOR_TEXT_HIGHLIGHT);if(this.also_color_this_floating_text!==null){this.also_color_this_floating_text.update_text_color(this.also_color_this_floating_text.text,COLOR_TEXT_HIGHLIGHT);}}else{this.wireframe.material.color.setHex(this.original_border_color);this.update_color(this.default_color);if(this.also_color_this_floating_text!==null){this.also_color_this_floating_text.update_text_color(this.also_color_this_floating_text.text,this.default_color);}}},state_change_engage:function(being_engaged_with){l(being_engaged_with);l(MANAGER_WORLD.player.is_engaged());l('@@@');if(being_engaged_with){if(this.type!==TYPE_BUTTON&&this.type!==TYPE_CHECK_BOX){MANAGER_WORLD.player.engage();}else{this.being_engaged_with=false;MANAGER_WORLD.player.disengage();}
}else{MANAGER_WORLD.player.disengage();}
l(MANAGER_WORLD.player.is_engaged());l('@@@');},get_text_length:function(){return this.dynamic_texture.getTextLength(this.text);},get_text_height:function(){return this.height;},clear_text:function(){this.dynamic_texture.clear();this.dynamic_texture.needsUpdate=true;},update_color:function(color){this.update_text_color(this.text,color);},update_text_color:function(text,color){this.text=text;switch(this.type){case TYPE_TITLE:case TYPE_CHECK_BOX:case TYPE_BUTTON:this.dynamic_texture.clear().drawText(this.text,this.width-this.get_text_length()/2,this.height+this.height/2,color);break;default:this.dynamic_texture.clear().drawText(this.text,0,this.height,color);}
...
this.geometry=new THREE.PlaneGeometry(this.width,this.height);this.dynamic_texture=new THREEx.DynamicTexture(this.width*2,this.height*2);if(this.type==TYPE_TITLE){this.dynamic_texture.context.font='Bold 40px Arial';}else{this.dynamic_texture.context.font='20px Arial';}

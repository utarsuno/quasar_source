...
inline void initialize_simulation_state() {
...
inline int can_place_buy_order() {
...
inline int can_place_sell_order() {
...
inline void place_buy_order(const float order_amount, const float order_price) {
    int i;
    for (i = 0; i < MAXIMUM_NUMBER_OF_BUY_ORDERS; i++) {
...
inline void place_sell_order(const float order_amount, const float order_price) {
    int i;
    for (i = 0; i < MAXIMUM_NUMBER_OF_SELL_ORDERS; i++) {
...
inline void check_if_any_buy_or_sell_orders_should_be_filled(const float current_price) {
    int i;
